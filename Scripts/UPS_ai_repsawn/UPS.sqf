...
//    (e.g. nul=[this,"town"] execVM "Scripts/UPS_ai_Respawn/ups.sqf")
//
//  Optional parameters:
...
if (isNil("KRON_UPS_INIT")) then {
	...
	KRON_HQ = _grpLogic createUnit ["Logic", [0,0,0], [], 0, "NONE"];
...
_closeenough=_closeenough*_closeenough;
...
// adjust cycle delay
_cycle = ["CYCLE:",5,_UCthis] call KRON_UPSgetArg;
...
if (_areatrigger!="NOTRIGGER") then {
	...
	if (isNil ("KRON_TRGFlag")) then {
		...
		if (_areatrigger!="SILENTTRIGGER") then {
			call compile format["%1 setTriggerStatements['count thislist<=%6', 'titletext [''SECTOR <%2> CLEARED'',''PLAIN''];''%2'' setmarkerpos [-%4,-%5];%3=true;', 'titletext [''SECTOR <%2> HAS BEEN RE-OCCUPIED'',''PLAIN''];''%2'' setmarkerpos [%4,%5];%3=false;']", _trgname,_areamarker,_flgname,_centerX,_centerY,_zoneempty];
		} else {
...
while {_loop} do {
	...
	if ((_issoldier) && ((count _enemies)>0) && !(_exit)) then {
		...
		if ((_shareinfo=="SHARE") && (behaviour _npc=="SAFE")) then {
			...
			{
				if (!(isNull _x) && (_npc distance _x<_sharedist) && (damage _x>.5)) exitWith {
		...
		_maxknowledge=0;
		{
			...
			if (KRON_UPS_Debug>10) then {player sidechat format["%1: %2",_grpidx,_knows]};
			if ((alive _x) && (_knows>0.2) && (_knows>_maxknowledge)) then {
				...
				if (KRON_UPS_Debug>0) then {player sidechat format["%1: %2",_grpidx,KRON_KnownEnemy]};
			};
	...
	if !(_newpos) then {
		...
		if (_waiting<=0) then {
			...
			if ((!_swimming) && ((_dist<=_closeenough) || (_totmove<.2) || (_dammchg>0.01) || (_timeontarget>_alerttime))) then {_makenewtarget=true;};
			...
			if (_makenewtarget) then {
				if ((_nomove=="NOMOVE") && (_timeontarget>_alerttime)) then {
	...
	if ((_waiting<=0) && _newpos) then {
		...
		_dist=0; _moved=0; _lastmove1=10; _waiting=-1;
		_newpos=false;
		_swimming=false;
